...
- **どちらが正しいという評価を書かないでください。** 委員の質問が鋭いとか、区の回答が
  ...
  そのまま伝えることに徹してください
- 会派名・政党名は書かないでください
- **委員は氏名だけ**にしてください（「高橋委員」→「高橋」）
- **理事者は議事録の表記をそのまま**書いてください
  （「左近士子ども家庭支援センター所長」はこのまま）。姓と役職の切れ目は
  区の組織名によって変わり、分けようとすると「左近士」を「左近 士子」の
  ように誤ります。区民に示す名前を間違えるくらいなら、続けて書くほうがよい
- 「以上でございます」「よろしくお願いいたします」のような定型のあいさつは、
  論点に含めないでください

## 発言番号

発言の先頭には \`［発言12］\` のように通し番号を付けてあります。
論点ごとに、**その論点で委員が最初に質問した発言の番号**を
firstUtteranceNumber に入れてください。記事はこの番号の順に並べます。

複数の委員が同じ論点を聞いていて1つにまとめた場合は、**いちばん早い**
発言の番号を入れてください。

## 書き方

区民が読む記事の文なので、事実だけを短く置いてください。

- **伝聞の言い回しを付けないでください。** 「〜との説明でした」「〜と答えました」
  「〜とのことです」は不要です。answer は区の回答であることが記事の作りで
  分かるので、「〜を行う予定」「〜は考えていない」のように内容で言い切ってください
- **question は委員の丁寧語をなぞらないでください。** 「〜を教えてください。」ではなく
  「〜は何か。」「〜はどうなるか。」のように、問いの形で短く書いてください
- 一文を長くしないでください
...
export function formatUtterances(
  ...
  for (const [index, utterance] of utterances.entries()) {
    ...
    // 番号は議事録の順そのもの。論点を時系列に並べ直すために使う
    const line = `［発言${index + 1}］${utterance.speaker}${mention}\n${body}`;
