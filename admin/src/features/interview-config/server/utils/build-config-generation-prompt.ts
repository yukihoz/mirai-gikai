...
報告資料に関する市民の意見を効果的に収集するためのインタビューテーマと質問を提案します。
...
${knowledgeSection}
## あなたの役割
この報告資料に合わせた **2種類のクイックリプライ選択肢** を生成してください。
...
## サンプル（あくまで参考。報告資料に合わせて差し替えること）
- topics サンプル（論点）: ${topicsEntry.sample_quick_replies.join(" / ")}
- stance サンプル（立場）: ${stanceEntry.sample_quick_replies.join(" / ")}

## 生成ガイドライン
- \`topics\`: 報告資料の主要論点の中から、市民が関心を持ちそうなテーマを5件。論点は報告資料の内容に固有のもの（条文・制度・対象など）とし、抽象論にしない。
- \`stance\`: この報告資料の影響を受けそうな立場・属性を5件。「一般市民として関心がある」を1件必ず含める。
...
## あなたの役割
確定済みの質問内容と報告資料情報をもとに、このインタビューで扱うテーマを提案してください。
テーマは、後段の分析で論点を集計・分類するためのラベルとして使われます。

## テーマ提案のガイドライン
- 確定質問で実際に聞かれている論点を漏れなくカバーする
- 報告資料の主要論点に対応するテーマにする
- 市民の生活や仕事への影響に関連する
- 具体的かつ分かりやすい表現にする
- 件数は報告資料内容と質問に応じて3〜6件程度を目安とする（固定ではない）
