...
/**
 * 初期テンプレート用: 報告資料ごとに Q1 / Q2 の quick_replies のみ LLM 生成する。
...
export const defaultQuestionsGenerationSchema = z.object({
  ...
  topics: z
    .array(z.string())
    .describe(
      "関心のあるテーマ（論点）の選択肢。報告資料固有の論点名を5件（例: 『AI利用』『罰則』『データ保護』のような論点）。立場・属性は絶対に入れない。"
    ),
  stance: z
    .array(z.string())
    .describe(
      "立場・関わり方の選択肢。報告資料の影響を受けそうな立場・属性を5件（例: 『仕事で〜』『〜の利用者』『〜の保護者』のような人の属性）。論点・テーマは絶対に入れない。汎用枠として『一般市民として関心がある』を必ず含めること。"
    ),
});
