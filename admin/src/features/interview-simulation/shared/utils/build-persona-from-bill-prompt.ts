...
/**
 * 報告資料内容からシミュ用ペルソナを生成させるプロンプトを構築する純粋関数。
 *
 * 過去の完了インタビューがない場合（レポート 0 件）でもシミュレートできるよう、
 * 報告資料内容・インタビューテーマ・任意のスタンス/役割ヒントから、インタビュー
...
export function buildPersonaFromBillPrompt(
  ...
  const stanceLine = stanceHint
    ...
    : "- **スタンス**: 報告資料の内容と、想定される役割から自然に導かれる立場を選んでください。for/against/neutral から適切なものを選択";
  ...
  const roleLine = hasRoleHint
    ...
    : "- **役割・属性**: 報告資料の影響を直接受けそうな当事者を想定してください。抽象的すぎる「国民全体」のような設定は避け、具体的な生活・業務文脈を持たせてください";

  return `あなたはインタビューシミュレータのための「当事者ペルソナ」を組み立てる設計者です。
以下の報告資料について、実際にインタビューされそうな**もっともらしい 1 人の当事者**のキャラクターシートを作ってください。
...
${stanceLine}
${roleLine}
- **knowledge_level**: この属性の人が報告資料についてどの程度事前に知っているかを推定（beginner / intermediate / expert）
...
- **key_concerns**: 報告資料について特に気にしている論点を 3〜5 件。抽象論ではなく、その立場の人が実感ベースで語れる具体論点
- **typical_response_length**: short / medium / long から選ぶ。この属性の人がインタビューで見せるであろう典型的な回答長
- **boundaries**: 答えたくない・話題を避けそうな領域があれば記載。なければ空配列
- **message_to_politicians**: このペルソナが今回の報告資料に関して政治家へ最終的に伝えたい核心メッセージを **3〜5 件の文字列配列** で。各項目は 1 文で完結し、項目間で意味的に独立させること（後段の満足度評価が項目ごとに引き出せたかを判定する）。内容としては、スタンスの結論 / 主な懸念 / 要望 / その立場特有の根拠 を組み合わせて、**具体的で検証可能**にする（抽象論のみは避ける）。

${
  hasRoleHint
    ? `## 重要
- **ユーザー指定の役割ヒントが最優先**。上の「役割・属性」指示に忠実であること。ヒントが「一般市民」「専業主婦」のような一見抽象的な属性でも、具体的な生活文脈（住んでいる地域 / 家族構成 / 日常で報告資料の影響を受けそうな場面 など）を肉付けしてリアルな 1 人に仕立てれば OK
- スタンスと役割は報告資料内容と矛盾しない範囲で現実的に。例: 規制される側が「規制緩和に賛成」は不自然
