...
function DynamicThumbnail({ title, meetingBody }: DynamicThumbnailProps) {
  ...
  return (
    <div
      className={`relative w-full h-48 rounded-xl overflow-hidden border border-gray-200 ${colorTheme.bg} ${colorTheme.text} flex flex-col items-center justify-center p-6 text-center shadow-inner`}
    >
...
export default function PatternsDemoPage() {
  const sampleBills = [
    ...
    {
      title: "中央区自転車の放置防止等に関する条例の一部を改正する条例",
      body: "地域活性化対策特別委員会",
    },
    {
      title: "中央区立児童館条例の一部を改正する条例",
      body: "子ども子育て・高齢者対策特別委員会",
    },
    { title: "子ども医療費助成に関する条例", body: "福祉保健委員会" },
    {
      title: "中央区防災会議条例の一部を改正する条例",
      body: "防災等安全対策特別委員会",
    },
    {
      title: "区立学校のＩＣＴ教育推進に関する補正予算",
      body: "区民文教委員会",
    },
    ...
    <div className="max-w-6xl mx-auto p-8 font-sans">
      <div className="mb-10">
        <h1 className="text-2xl font-bold mb-4">
          サムネイル自動生成のサンプル検証 (20柄 × 会議体カラー)
        </h1>
        <p className="text-gray-600 leading-relaxed mb-4">
          以下のデモでは、**議案の名前（ハッシュ値）によって 20種類
          のパターンから1つが選ばれ**、**背景の「色（テーマ）」は所属する会議体（委員会や定例会）によって変化する**
          仕組みを実装しています。
        </p>
        <div className="bg-gray-50 p-4 rounded-lg text-sm border">
          <h3 className="font-bold mb-2">会議体別のカラーマッピング手法</h3>
          <p className="mb-2">
            会議体ごとに Tailwind CSS のクラス（背景色 <code>bg-*-50</code> と
            文字色 <code>text-*-500/20</code>{" "}
            の組み合わせ）を持たせた辞書（Dictionary）形式で管理しています。SVG側は{" "}
            <code>stroke="currentColor"</code> または{" "}
            <code>fill="currentColor"</code>{" "}
            で描画されるため、親要素の色に応じて自動的に模様の色が変わります。
          </p>
          <ul className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
            <li>
              <span className="inline-block w-3 h-3 bg-orange-100 border border-orange-300 mr-1"></span>
              区民文教: オレンジ
            </li>
            <li>
              <span className="inline-block w-3 h-3 bg-emerald-100 border border-emerald-300 mr-1"></span>
              環境建設: エメラルド
            </li>
            <li>
              <span className="inline-block w-3 h-3 bg-rose-100 border border-rose-300 mr-1"></span>
              福祉保健: ピンク/レッド
            </li>
            <li>
              <span className="inline-block w-3 h-3 bg-slate-200 border border-slate-400 mr-1"></span>
              企画総務: グレー
            </li>
            <li>
              <span className="inline-block w-3 h-3 bg-yellow-100 border border-yellow-400 mr-1"></span>
              予算決算: イエロー
            </li>
            <li>
          ...
          {sampleBills.map((bill) => (
            ...
            >
              ...
              <div className="text-xs text-center text-gray-500 mt-2 space-y-1">
                <p>
                  会議体による色:{" "}
                  <strong className="text-gray-700">{bill.body}</strong>
                </p>
                <p>
                  議案名ハッシュによる柄:{" "}
                  <strong>Pattern {(hashString(bill.title) % 20) + 1}</strong>
                </p>
              </div>
            </div>
          ))}
