...
## AIインタビュー機能について
みらい議会には「AIインタビュー」機能があります。これは報告資料ごとに提供される機能で、ユーザーがAIインタビュアーと対話形式で報告資料に対する意見や知見を共有できる仕組みです。インタビュー結果は分析・レポート化され、政策議論に活用されます。
`;

const INTERVIEW_AWARENESS_PROMPT_BILL = `${INTERVIEW_AWARENESS_BASE}
この報告資料のインタビュー機能が現在利用可能かどうかは状況によって異なります。インタビューについて質問された場合は、この機能の存在を説明した上で、報告資料詳細ページでインタビューへの案内が表示されているか確認するよう案内してください。
`;

const INTERVIEW_AWARENESS_PROMPT_HOME = `${INTERVIEW_AWARENESS_BASE}
インタビューについて質問された場合は、この機能の存在を説明した上で、利用可否は報告資料ごとに異なるため、興味のある報告資料の詳細ページでインタビューへの案内が表示されているか確認するよう案内してください。
`;

const INTERVIEW_SUGGESTION_PROMPT = `

## AIインタビュー提案について
この報告資料にはAIインタビュー機能があります。以下の条件に該当する場合、通常のテキスト応答と併せて suggest_interview ツールを呼び出してください。
ただし、1つの会話の中で suggest_interview ツールを呼び出すのは1回のみとしてください。

### suggest_interview を呼び出す条件:
- ユーザーがこの報告資料の当事者や関係者であることがわかった場合
- ユーザーがこの報告資料について専門的な知識を持っていると判断できる場合
- ユーザーがこの報告資料に対して具体的な提言や意見を述べた場合
...
function buildTools(shouldSuggestInterview: boolean) {
  ...
  if (shouldSuggestInterview) {
    tools[SUGGEST_INTERVIEW_TOOL_NAME] = tool({
      description:
        "ユーザーが報告資料の当事者・有識者であると判断された場合、またはインタビューについて聞かれた場合に呼び出す。通常のテキスト応答と同時に呼び出すこと。",
