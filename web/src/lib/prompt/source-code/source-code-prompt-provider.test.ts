...
describe("SourceCodePromptProvider", () => {
  ...
  it("top-chat-system プロンプトを変数付きで返す", async () => {
    const result = await provider.getPrompt("top-chat-system", {
      billSummary: '[{"id":"1","name":"テスト報告資料"}]',
    });

    expect(result.content).toContain(env.siteShortName);
    expect(result.content).toContain('[{"id":"1","name":"テスト報告資料"}]');
  });

  it("bill-chat-system-normal プロンプトを変数付きで返す", async () => {
    const result = await provider.getPrompt("bill-chat-system-normal", {
      billName: "テスト報告資料",
      billTitle: "テスト報告資料のタイトル",
      billSummary: "テスト報告資料の要約",
      billContent: "テスト報告資料の詳細内容",
    });

    expect(result.content).toContain(env.siteShortName);
    expect(result.content).toContain("テスト報告資料");
    expect(result.content).toContain("テスト報告資料のタイトル");
    expect(result.content).toContain("テスト報告資料の要約");
    expect(result.content).toContain("テスト報告資料の詳細内容");
