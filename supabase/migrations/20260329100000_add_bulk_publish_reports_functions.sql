...
create function bulk_publish_reports(
  ...
  with updated as (
    ...
    where interview_session_id in (
      ...
      where interview_config_id = p_config_id
    )
    and is_public_by_user = true
    and is_public_by_admin = false
    and moderation_score is not null
    and moderation_score <= p_max_moderation_score
    and total_content_richness is not null
    and total_content_richness >= p_min_content_richness
    returning id
  )
